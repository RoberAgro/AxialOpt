function turbine_data = evaluate_cascade_model(k,vel_in,angle_in,x_cascade,turbine_data)

% This function contains the mean-line model of each cascade
...
[u_in,v_in,v_t_in,v_m_in,w_in,w_t_in,w_m_in,alpha_in,beta_in] = ...
    evaluate_velocity_triangle_in(u_in,v_in,alpha_in);
...
[u_out,v_out,v_t_out,v_m_out,w_out,w_t_out,w_m_out,alpha_out,beta_out] =...
    evaluate_velocity_triangle_out(u_out,w_out,beta_out);
...
if k == 1
    ...
    T0_in = prop_calculation('T','P',p0_in,'H',h0_in,fluid);
    s_in  = prop_calculation('S','P',p0_in,'H',h0_in,fluid);
    h_in  = h0_in-v_in^2/2;
    p_in  = p_hs_flash(h_in,s_in,fluid,p_min,p_max);
    T_in  = prop_calculation('T','P',p_in,'S',s_in,fluid);
    d_in = prop_calculation('D','P',p_in,'H',h_in,fluid);
    Z_in = prop_calculation('Z','P',p_in,'H',h_in,fluid);
    a_in = compute_speed_of_sound(p_in,'H',h_in,fluid);
    mu_in = compute_viscosity(p_in,'H',h_in,fluid);
...
elseif k > 1
    ...
    T0rel_in = prop_calculation('T','P',p0rel_in,'H',h0rel_in,fluid);
...
T0rel_out = prop_calculation('T','P',p0rel_out,'H',h0rel_out,fluid);
p_out = p_hs_flash(h_out,s_out,fluid,p_min,p_max);  % s_out is a DoF
T_out = prop_calculation('T','P',p_out,'H',h_out,fluid);
d_out = prop_calculation('D','P',p_out,'H',h_out,fluid);
Z_out = prop_calculation('Z','P',p_out,'H',h_out,fluid);
