function [] = plot_view_axial_radial(turbine_data,my_filename,my_path,save)

% Axial-Radial view of the turbine
...
r_1_in = turbine_data.plane(end).r_h;
r_2_in = turbine_data.plane(end).r_t;
...
casing_x = [    -3*dy   casing_x   L_turb+5*dy         L_turb+5*dy             L_turb       -3*dy       -3*dy];
casing_y = [casing_y(1)   casing_y   r_2_in    r_2_in+dy   r_2_in+dy   r_2_in+dy   casing_y(1)];
...
outlet_x = [     0     4*dy        4*dy   bearing_x           0        0] + L_start;
outlet_y = [r_1_in   r_1_in   r_1_in-dy   bearing_y   r_1_in-dy   r_1_in];
