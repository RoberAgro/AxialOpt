function [] = plot_diagram_hs(turbine_data,plot_satline,my_filename,my_path,save)
...
% Define axis limits
delta_h = max(h_turbine) - min(h_turbine);
delta_s = max(s_turbine) - min(s_turbine);
h_minplot = min(h_turbine) - delta_h/4;
h_maxplot = max(h_turbine) + delta_h/4;
...
if strcmp(plot_satline,'yes') == 1
    [h_sat, s_sat] = compute_sat_line(fluid,T_trip,T_crit,'H','S',200);
    plot(s_sat/1000,h_sat/1000,'k','LineWidth',0.75)
    h_minplot = min(h_minplot, min(h_sat));
    h_maxplot = 1.1*max(h_maxplot, max(h_sat));
...
for i = 1:length(p_turbine)
    [h_isobar, s_isobar, ~] = compute_isobar(p_turbine(i),fluid,200);
    plot(s_isobar/1000, h_isobar/1000, 'k')
end

% Plot the isobars corresponding to the exhaust pressure
[h_isobar, s_isobar, ~] = compute_isobar(p_out,fluid,200);
plot(s_isobar/1000, h_isobar/1000, 'b')

% Plot the critical point
h_crit = prop_calculation('H','T',T_crit,'P',p_crit,fluid);
