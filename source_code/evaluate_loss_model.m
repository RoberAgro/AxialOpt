function turbine_data = evaluate_loss_model(k,turbine_data,parameters)
...
if strcmp(loss_system,'AM') == 1
    ...
    [Y_loss,Y_p,Y_s,Y_cl,Y_te] = evaluate_loss_model_AM(cascade,plane_in,plane_out);
elseif strcmp(loss_system,'DC') == 1   
    % Dunham-Came loss system
    [Y_loss,Y_p,Y_s,Y_cl,Y_te] = evaluate_loss_model_DC(cascade,plane_in,plane_out);
elseif strcmp(loss_system,'KO') == 1
    % Kacker-Okapuu loss system
    [Y_loss,Y_p,Y_s,Y_cl,Y_te] = evaluate_loss_model_KO(cascade,plane_in,plane_out);
else
