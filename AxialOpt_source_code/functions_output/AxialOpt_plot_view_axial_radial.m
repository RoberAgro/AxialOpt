function [] = AxialOpt_plot_view_axial_radial(turbine_data,my_filename,my_path,save)
%% Axial-Radial view of the axial turbine
...
x_shaft = [-5*dy+curvy_line_x  L+7*dy+curvy_line_x(end:-1:1) -5*dy];
...
casing_x = [    -3*dy   casing_x   L+5*dy         L+5*dy             L       -3*dy       -3*dy];
