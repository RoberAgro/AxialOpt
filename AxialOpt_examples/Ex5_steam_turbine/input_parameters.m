...
fluid = 'water';                                                           % Name used by REFPROP
T0_in = 480+273.15;                                                        % Inlet stagnation temperature in (K) (alternatively provide the stagnation enthalpy if the expansion starts in the two-phase region)
p0_in = 8000.0;                                                            % Inlet stagnation pressure in (kPa)
h0_in = refpropm('h','T',T0_in,'p',p0_in,fluid);                           % Inlet stagnation enthalpy in (J/kg)   
p_out = 700.0;                                                             % Outlet static pressure in (Pa)

% Define the mass flow rate
mass_flow = 65.6389;                                                       % Mass flow rate (kg/s)
...
AR = 1.500;                                                                % Diffuser area ratio (set a value close to one for the no-diffuser case)
...
% Hub-to-tip ratio
constraints.r_ht.min = 0.750;                                              % Kacker-Okapuu correlation holds for values as low as 0.500. Saravanamuttoo discusses ratios as low as 0.50.
