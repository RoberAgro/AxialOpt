...
% Values from the R125 1-stage axial turbine example from chapter 9 of:
% Organic Rankine Cycle (ORC) Power Systems, Macchi and Astolfi (2017)
fluid = 'hexane';                                                          % Name used by REFPROP
T0_in = 155+273.15;                                                        % Inlet stagnation temperature in (K) (alternatively provide the stagnation enthalpy if the expansion starts in the two-phase region)
p0_in = refpropm('p','T',T0_in-0.1,'q',1.00,fluid);                        % Inlet stagnation pressure in (kPa). Expansion from saturated vapor
h0_in = refpropm('h','T',T0_in,'p',p0_in,fluid);                           % Inlet stagnation enthalpy in (J/kg)   
p_out = 0.250*100;                                                         % Outlet static pressure in (Pa)

% Define the mass flow rate
% mass_flow = 1000.0;                                                        % Mass flow rate (kg/s)
...
% Specific speed
w_s = 0.5;                                                                % Easier to guess than the angular speed
...
vel_out(1) = 0.5*1/sqrt(2*n_stages);
...
constraints.r_ht.max = 0.950;                                              % High values imply large secondary losses
constraints.r_ht.applied = 'yes';
constraints.r_ht.ref = 1.00;

% Degree of reaction 
constraints.reaction.min = 0.10;                                          % Avoid pressure increase in rotor cascades  (R>0)
constraints.reaction.max = 0.90;                                          % Avoid pressure increase in stator cascades (R<1)
...
p_min = 0.75*p_out;                                                         % Minimum pressure
p_max = 1.5*p0_in;                                                         % Maximum pressure
