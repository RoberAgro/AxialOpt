...
fluid = 'CO2';                                                             % Name used by REFPROP
T0_in = 550.00+273.15;                                                     % Inlet stagnation temperature in (K) (alternatively provide the stagnation enthalpy if the expansion starts in the two-phase region)
p0_in = 19827.95;                                                          % Inlet stagnation pressure in (kPa)
h0_in = refpropm('h','T',T0_in,'p',p0_in,fluid);                           % Inlet stagnation enthalpy in (J/kg)   
p_out = 7901.16;                                                           % Outlet static pressure in (Pa)

% Define the mass flow rate
mass_flow = 3209.0;                                                        % Mass flow rate (kg/s)
